...
<img width="1104" height="619" alt="image" src="https://github.com/user-attachments/assets/6a6d0a6a-df53-4664-9e3a-e916b5d18c61" />
...
* выполнение health checks зависимых компонентов.
  <img width="1091" height="73" alt="image" src="https://github.com/user-attachments/assets/637b484b-d569-4f7d-80a5-023185157fc5" />
...
Изначально использовались стандартные Docker-образы Python, однако при работе в окружении WSL  возникли проблемы с размером образов и временем сборки. В результате было принято решение перейти на образ python:3.11-slim, что позволило
уменьшить размер Docker-образов и повысить стабильность запуска сервисов в связке WSL + Docker.

Была сложность с реализацией параллельных запросов к микросервисам из API Gateway. Удалось решить используя асинхронные возможности FastAPI и корректную обработку таймаутов
